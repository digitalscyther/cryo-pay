...
pub async fn apply(state: &Arc<AppState>, payment: &Payment) -> Result<(), AppError> {
    state.db.set_payment_paid(&payment.id).await?;

    let payable = serde_json::from_value::<Payable>(payment.data.clone())
        .map_err(|err| AppError::Internal(utils::make_err(Box::new(err), "parse payment")))?;

    match payable {
        Payable::Donation(_) => {}
        Payable::Subscription(subscription) => {
            let user_id = payment.user_id.ok_or_else(|| AppError::Internal("subscription user not detected".to_string()))?;
