...
### Fixed
- Swagger UI: added `/api` server prefix so requests route through nginx to the backend instead of the frontend
- Swagger UI: defined `jwt_cookie` security scheme (cookie `ApiKey`) so the lock icon works correctly

## [1.1.3] - 2026-03-09

### Fixed
- Frontend: auth state now persists across page navigation — replaced unreadable HttpOnly cookie check with a `GET /user` API call on app load

## [1.1.2] - 2026-03-09

### Fixed
- OpenAPI spec: `BigDecimal` amount fields annotated with `#[schema(value_type = String)]` so Swagger UI resolves them without a `$ref` error

## [1.1.1] - 2026-03-09

### Added
- "API Docs" navbar link and "API Reference" section in `/docs` linking to Swagger UI at `/swagger-ui/`

### Fixed
- Docker build: added `curl` to builder image so `utoipa-swagger-ui` can download assets at compile time
- Nginx: added `/swagger-ui/` and `/api-docs/` locations so SwaggerUI spec loads correctly through the reverse proxy
...
[1.1.2]: https://github.com/digitalscyther/cryo-pay/compare/v1.1.1...v1.1.2
[1.1.1]: https://github.com/digitalscyther/cryo-pay/compare/v1.1.0...v1.1.1
